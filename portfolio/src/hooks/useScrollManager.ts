...
import { useScroll, animate, MotionValue, useTransform, useMotionValueEvent } from 'motion/react';
...
export function useScrollManager({
   ...
   * Works with both container scroll and window scroll
   * @returns {number | null} Index of the active section or null if not applicable
   */
  const calculateActiveSection = useCallback(() => {
    if (!sectionRefs?.current) return null;
    ...
    sections.forEach((section, index) => {
      // For window scroll, use getBoundingClientRect; for container scroll, use offsetTop
  ...
  }, [scrollY, scrollYProgress, debounceMs, calculateActiveSection, calculateSectionMetrics, scrollState.activeSection, sectionNames]);
  ...
  }, [scrollY, handleScroll, containerRef]);
  ...
  const scrollToSection = useCallback((index: number) => {
    ...
    if (!containerRef?.current) {
      // Use window scroll - calculate position relative to viewport
  ...
  const scrollToTop = useCallback(() => {
    if (!containerRef?.current) {
      // Use window scroll with smooth animation
    ...
    // Use container scroll with smooth animation
    const container = containerRef.current;
  ...
  return {
    ...
    scrollToSection,
    scrollToTop,
    scrollProgress: scrollYProgress, // Overall scroll progress (0-1) - MotionValue
    scrollPosition: scrollY, // Current scroll position - MotionValue
    scrollY, // Raw scrollY MotionValue for advanced parallax use
    scrollYProgress, // Raw scrollYProgress MotionValue for advanced parallax use
    containerRef,
    sectionRefs,
    sectionNames,
